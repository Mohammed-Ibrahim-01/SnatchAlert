...
FILES MODIFIED:
- serializers.py: Added 4 new serializers (DeviceAlert, MyDeviceAlertsResponse, 
  IMEICheckLog, IMEICheckHistoryResponse)
- views.py: Added serializer_class attributes to both problematic views
...
class IncidentFactCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # AUTO-REGISTER IMEI if stolen item is a phone with IMEI
        if (stolen_item and 
...
class IMEIRegistrySerializer(serializers.ModelSerializer):
    """
    Serializer for IMEI registry - used for listing and updating IMEI records.
